...
pen.define("cdf/lib/CCC/pvc-d1.0", [ "cdf/lib/CCC/def", "cdf/lib/CCC/protovis", "cdf/jquery", "cdf/lib/CCC/tipsy" ], function(def, pv, $) {
    ...
    function dim_onDatumVisibleChanged(datum, visible) {
        ...
        if (!this._disposed && (map = this._atomVisibleDatumsCount)) {
            ...
            DEBUG && (def.hasOwn(this._atomsByKey, key) || def.assert("Atom must exist in this dimension."));
            var count = map[key];
            DEBUG && (visible || count > 0 || def.assert("Must have had accounted for at least one visible datum."));
    ...
    var DEBUG = 1, pvc = def.globalSpace("pvc", {}), pvc_arraySlice = Array.prototype.slice;
        ...
        if (null != value && "" !== value) {
            switch (typeof value) {
              ...
              case "string":
                ...
                if (match) {
                    var n = +match[1];
                    if (!isNaN(n) && match[2]) return n >= 0 ? new pvc_PercentValue(n / 100) : n;
                }
                break;
    ...
    def.type("pvc.GridDockingPanel", pvc.BasePanel).add({
        ...
        _calcLayout: function(layoutInfo) {
            function layoutCycle(remTimes, iteration) {
                ...
                try {
                    for (var paddingsChanged, breakAndRepeat, canChange = layoutInfo.canChange !== !1 && !isDisasterRecovery && remTimes > 0, ownPaddingsChanged = !1, index = 0, count = sideChildren.length; count > index; ) {
    ...
    def.type("pvc.BaseChart", pvc.Abstract).add(pvc.visual.Interactive).init(function(options) {
        ...
        if (pvc.debug >= 3 && !parent && originalOptions) {
            this._info("OPTIONS:\n", originalOptions);
            pvc.debug >= 5 && this._trace(pvc.stringify(originalOptions, {
                ownOnly: !1,
                funs: !0
            }));
        }
        ...
        _processDataOptions: function(options) {
            ...
            if (plot2) {
                ...
                plot2Series || (plot2SeriesIndexes = pvc.parseDistinctIndexArray(plot2SeriesIndexes, -1/0) || -1);
            }
    ...
    pvc.BaseChart.add({
        ...
        _createTranslation: function(complexTypeProj, dimsOptions, dataPartDimName) {
            ...
            pvc.debug >= 3 && (this._log(translation.logSource()), this._log(translation.logTranslatorType()));
        ...
        _createPartData: function(baseData, partRole, dataPartValues) {
            var dataPartDimName = partRole.lastDimensionName(), dataPartAtoms = baseData.dimensions(dataPartDimName).getDistinctAtoms(def.array.to(dataPartValues)), where = data_whereSpecPredicate([ def.set({}, dataPartDimName, dataPartAtoms) ]);
    ...
    var pvc_CartesianAxis = def.type("pvc.visual.CartesianAxis", pvc_Axis).init(function(chart, type, index, keyArgs) {
    ...
    def.type("pvc.visual.CartesianPlot", pvc.visual.Plot).init(function(chart, keyArgs) {
    ...
    def.type("pvc.CartesianAbstract", pvc.BaseChart).init(function(options) {
        ...
        _initAxesEnd: function() {
            this._axisOffsetPaddings = this.parent ? this.parent._axisOffsetPaddings : this._calcAxesOffsetPaddings();
            this.base();
        },
        _calcAxesOffsetPaddings: function() {
            ...
            var pctPaddings = {}, hasAny = !1, chartAxes = this.axesByType;
    ...
    def.type("cdo.TranslationOper").init(function(chart, complexTypeProj, source, metadata, options) {
        ...
        _logItemBefore: function(vitem) {
            if (this._logItemCount < 10) return pvc.log("virtual item [" + this._logItemCount++ + "]: " + pvc.stringify(vitem)), 
            !0;
            pvc.log("...");
    ...
    var numForm_cache = {}, numForm_cacheCount = 0, dateForm = cdo.dateFormat = function() {
    ...
    def.type("pvc.visual.PiePlot", pvc.visual.Plot).init(function(chart, keyArgs) {
        this.base(chart, keyArgs);
    ...
    def.type("pvc.visual.WaterfallPlot", pvc.visual.BarPlotAbstract).init(function(chart, keyArgs) {
        this.base(chart, keyArgs);
    ...
    def.type("pvc.visual.MetricPointSizeAxis", pvc.visual.SizeAxis).add({
        _getOptionsDefinition: function() {
    ...
    def.type("pvc.HeatGridPanel", pvc.CategoricalAbstractPanel).init(function(chart, parent, plot, options) {
        ...
        _createShapesHeatMap: function(cellSize, wrapper, hasColor, hasSize) {
            var me = this, areaRange = me._calcDotAreaRange(cellSize), keyArgs = {
                extensionId: "dot",
                freePosition: !0,
                activeSeriesAware: !1,
                wrapper: wrapper,
            ...
            hasSize ? me.axes.size.setScaleRange(areaRange) : pvDot.sign.override("defaultSize", def.fun.constant(areaRange.max));
            return pvDot;
        },
    ...
    def.type("pvc.visual.TreemapPlot", pvc.visual.Plot).init(function(chart, keyArgs) {
        this.base(chart, keyArgs);
    ...
    def.type("pvc.visual.SunburstPlot", pvc.visual.Plot).init(function(chart, keyArgs) {
        this.base(chart, keyArgs);

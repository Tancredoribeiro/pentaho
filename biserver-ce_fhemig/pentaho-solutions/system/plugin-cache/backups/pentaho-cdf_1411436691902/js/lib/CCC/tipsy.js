    ...
    !function() {
        ...
        pv.Behavior.tipsy = function(opts) {
            ...
            function getRealIndex(scene, index) {
                return "function" == typeof _mark.getNearestInstanceToMouse ? _mark.getNearestInstanceToMouse(scene, index) : index;
            }
            ...
            function onTargetElemMouseMove(ev) {
                if ($fakeTipTarget) if (null != prevMouseX && Math.abs(ev.clientX - prevMouseX) < 3 && Math.abs(ev.clientY - prevMouseY) < 3) _tip.debug >= 20 && _tip.log("[TIPSY] #" + _tipsyId + " mousemove too close"); else {
            ...
            var $fakeTipTarget, $targetElem, prevMouseX, prevMouseY, _renderId, _index, _scenes, _mark, id, $canvas, sharedTipsyInfo, nextOperationId = 0, delayOut = opts.delayOut, usesPoint = opts.usesPoint, isEnabled = opts.isEnabled;
            opts.delayOut = 0;
